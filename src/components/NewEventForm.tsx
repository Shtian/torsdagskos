...
export default function NewEventForm({
  ...
  const onSubmit = async (event: FormSubmitEvent) => {
    ...
    if (!parsedForm.success) {
      ...
      (Object.keys(nextErrors) as FieldName[]).forEach((fieldName) => {
  ...
  const feedbackClassName =
    feedbackType === 'idle'
      ? 'hidden rounded-md border px-4 py-3'
      : feedbackType === 'loading'
        ? 'block rounded-md border border-border bg-muted/50 px-4 py-3'
        : feedbackType === 'success'
          ? 'block rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3'
          : 'block rounded-md border border-destructive/30 bg-destructive/10 px-4 py-3';
