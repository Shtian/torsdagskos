...
- For React-island interaction tests on `/demo/shadcn-island`, wait for `data-hydrated="true"` on `page.getByTestId('shadcn-island')` before clicking tabs/select/dialog/dropdown triggers to avoid pre-hydration flakiness.
...
- For settings notification island interactions, wait for `[data-settings-notifications="true"][data-hydrated="true"]` before permission-button assertions so tests do not race pre-hydration handlers.
