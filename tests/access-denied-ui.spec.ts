...
test.describe('Access denied UI @unauth', () => {
  test('renders migrated shell with unchanged status and sign-in action', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId('access-denied-kicker')).toHaveText(
      'Tilgang begrenset',
    );
    await expect(
    ...
    await expect(page.getByTestId('access-denied-message')).toContainText(
      'Du trenger en kontoinvitasjon for å fortsette.',
    );
  ...
  test('shows requested path context when redirected from protected route', async ({
    page,
  }) => {
