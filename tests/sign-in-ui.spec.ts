...
test.describe('Sign-in wrapper UI @unauth', () => {
  test('renders migrated shell with Clerk sign-in intact', async ({ page }) => {
    ...
    await expect(page.getByTestId('sign-in-kicker')).toHaveText(
      'Velkommen tilbake',
    );
    await expect(
      page.getByRole('heading', { name: 'Logg inn på Torsdagskos', level: 1 }),
    ).toBeVisible();
    await expect(page.getByTestId('sign-in-description')).toContainText(
      'Bruk den inviterte kontoen din',
    );

    await expect(page.getByRole('heading', { name: /sign in/i })).toBeVisible();
    await expect(
      page.getByRole('link', {
        name: 'Trenger du en konto? Gå til registrering',
      }),
